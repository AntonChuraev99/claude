...
| **L2** `bug-pattern-reviewer` (агент) | суждение по red-flags тронутых областей; **поднимается только находкой `static`** (2026-08-31) | **runtime** — гонки, insets, анимация, video (grep флагует, подтверждает прогон) | 1 субагент |
...
- id: firebase-functions-region   # уникальный kebab-case
  ...
  narrowed_since: '2026-08-31'     # опц. дата последнего сужения детектора (см. «Прополка»)
  ...
  detect:                          # static/runtime:
    ...
    requires: 'ModalBottomSheet'   #   file-level — пропустить файл, если контекста правила НЕТ (grep)
...
Типы детекторов:
- **grep** — `has` (есть на строке) → находка; `unless` гасит строку; `lacks` гасит весь файл, если guard уже есть; `requires` гасит весь файл, если в нём **нет** контекста, о котором правило (зеркало `lacks`).

  `requires` — основной инструмент против ложных срабатываний у правил, чей `message` описывает узкий контекст, а `has` ловит широкий паттерн. Признак, что правило просит `requires`: в `message` есть «внутри X», «когда рядом Y», «в связке с Z», а `detect.has` про X/Y/Z ничего не знает. Замер 2026-08-30: правила без такого сужения давали 84–100% ложных при нуле блокировок.
- **glob** — срабатывает просто потому, что файл по `globs` попал в diff (напр. `.svg` в commonMain).

## Прополка (движок, который не даёт реестру зарасти шумом)

Компаундинг добавляет правила, прополка убирает те, что не окупились. Секцию **«Прополка — кандидаты на пересмотр»** печатает `stats.py` в `~/.claude/stats/review-rules.md` по порогу: **≥200 срабатываний, 0 блокировок, ≥3 суждений L2, est-FP ≥90%**. Реестр чистится по данным, а не по памяти.

По каждому кандидату — одно из: добавить `requires`/`lacks`, сузить `globs`, понизить `severity`, удалить.

**Сузил детектор — поставь `narrowed_since: 'YYYY-MM-DD'`.** С этой датой прополка считает правило только по тому, что оно нафайрило **после** сужения: старая статистика описывает уже несуществующий детектор, и без отсечки правило вечно висело бы в кандидатах со своей исторической FP.

**У сужённых правил критерий — объём, а не FP.** L2 поднимается только находкой `static`, а сужают почти всегда `runtime`: вердиктов confirmed/dismissed такое правило больше не набирает, и ждать их бессмысленно. Поэтому сужённое правило, которое после сужения снова набрало ≥200 срабатываний, попадает в блок **«Сужены, но объём не упал»** — сужение не сработало, сужать точнее или удалять. Упало ниже порога — правило в «ждут новых данных», и это уже сигнал, что сужение работает.

**Сужение, которое глушит сам баг, — хуже отсутствия сужения.** Проверять по собственным `message` и `fix` правила: если `message` называет два носителя условия («AnimatedVisibility/if»), а `requires` покрывает один — вторая половина класса выпадает молча. Если `message` говорит «X здесь нет вовсе», файловый `lacks: X` гасит правило ровно там, где оно нужно. Три такие правки 2026-08-31 были отловлены ревью и откачены в тот же день; в правилах остались комментарии «НЕ сужать так» с причиной — они дороже самого сужения.
