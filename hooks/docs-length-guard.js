...
//
// Enforcement для потолков из references/doc-length-budget.md (читает главный по скиллу
// doc-task; до 2026-09-15 — субагент doc-writer). Правило само себя не удержало: замер
// 2026-08-06 показал, что первый же документ, написанный ПОСЛЕ введения потолков,
// превысил их в 4 раза и не поставил обязательную строку `> Объём: <N> слов — <причина>`.
// Прозаический потолок писателю не виден, арифметику должен считать не он.
//
...
// Копия таблицы из references/doc-length-budget.md. Расходятся эти два места — правит человек:
...
Потолки заданы в ~/.claude/references/doc-length-budget.md: документ читают
