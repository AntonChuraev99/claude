# Реестр кредов проектов — шаблон

Скопировать в `project-credentials.local.md` (gitignored) и заполнить своими значениями.

Файл читает `hooks/credentials-guard.ps1` перед выполнением команд, которые меняют состояние во внешних сервисах (`gcloud`, `wrangler`, `firebase`, `gh`, `adb`). Задача — не дать задеплоить в чужой аккаунт из соседнего проекта.

## Формат

Одна таблица, строка на репозиторий. `repo_path` — абсолютный путь к корню репозитория либо его последний сегмент (сравнение идёт по вхождению, регистр не важен). Пустая ячейка означает «сервис в этом проекте не используется».

| repo_path | account | gcp_project | cf_account_id | firebase_project | play_package | git_remote |
|---|---|---|---|---|---|---|
| C:\Users\YOUR_USERNAME\StudioProjects\<your-project> | personal@example.com | your-gcp-project-id | 0000000000000000000000000000000 | your-firebase-project | com.example.app | git@gitlab.com:org/repo.git |
| C:\Users\YOUR_USERNAME\Documents\<another-project> | work@example.com | another-gcp-id |  | another-firebase | com.example.other | git@github.com:org/other.git |

`account` — почта, под которой должен идти деплой этого проекта. Это главная колонка: остальные идентификаторы уточняют, но именно смена аккаунта приводит к необратимой ошибке.

## Подстановка аккаунта в команды

Тот же реестр читает `hooks/account-align.js` — и не только сверяет, а подставляет аккаунт в команду до выполнения:

| инструмент | что добавляется |
|---|---|
| `gcloud` | глобальные флаги `--account=<account> --project=<gcp_project>` |
| `firebase` | глобальный флаг `--account=<account>` |

Поэтому переключать активную конфигурацию руками не нужно: глобальное состояние CLI не меняется вовсе, а значит параллельные сессии в разных проектах друг другу не мешают и git worktree ничего не ломает.

Флаги, а не переменные окружения (`CLOUDSDK_CORE_ACCOUNT=… gcloud …`), хотя те тоже работают: префикс переменных пишется по-разному для bash и PowerShell, а в PowerShell присваивание отделяется `;` и **разрывает цепочку** — `cd X && $env:A='v'; gcloud deploy` выполнил бы деплой даже после провала `cd`. Флаг одинаков в обоих шеллах и живёт внутри своей команды.

Каталог команды определяется по последней `cd` в цепочке (относительный путь резолвится от текущего каталога), иначе — рабочий каталог сессии.

**Не подставляется:**

- `gcloud auth login|revoke|application-default`, `firebase login|logout` — ими пользователь чинит токен;
- команды с уже указанным `--account` или `--configuration` — названа учётка, значит выбор сознательный (для `gcloud` подстановка отменяется целиком);
- `--project` отменяет подстановку **только проекта**: учётка им не названа, и без `--account` команда ушла бы под глобально активной почтой;
- каталоги, которых нет в таблице выше.

Пустая колонка `gcp_project` означает «проект не подставляем» — аккаунт подставится всё равно.

**Не покрыто:** MCP-тулы Firebase (у них своя авторизация) и любые тулы, кроме `Bash` и `PowerShell`. Их не видит и `credentials-guard` — перед записью через MCP аккаунт сверяет главный сам по этому реестру.

`gsutil` — legacy, не использовать: своих флагов аккаунта не имеет, и Google убирает его из состава Cloud CLI после марта 2027. Для Cloud Storage — `gcloud storage …`, его хук выравнивает как обычный `gcloud`; старый бинарь, если стоит отдельно, guard сверяет по-прежнему.

## Проба живости на старте сессии

`hooks/credentials-digest.ps1` на `SessionStart` (кроме `compact`) для строк, где заполнена `gcp_project` или `firebase_project`, запускает `gcloud auth print-access-token --account=<account>` (1–9 с; таймаут 12 с общий на процесс и оба потока, таймаут самого хука в `settings.json` — 20 с; живой вердикт кешируется на 30 мин в `stats/credentials-probe/`, любой другой вердикт кеш стирает) и сразу под заголовком дайджеста, до таблицы, сообщает вердикт: сессия живая · просрочена (Google Workspace «Google Cloud session control» — штатная повторная аутентификация раз в 1–24 ч, лечится только `! gcloud auth login <account>` руками пользователя) · токен отозван · аккаунт не залогинен. Пустые обе колонки — пробы нет: gcloud проекту не нужен.

## Как это работает

1. Команда матчит опасный шаблон — деплой, публикация, запись секрета, релиз. Обёртки (`npx`, `pnpm dlx`, `bash -c`, префикс `VAR=1`) распознаются: `npx wrangler deploy` — такой же деплой, как прямой вызов.
2. Хук определяет каталог команды: последняя `cd`/`pushd` в цепочке, иначе рабочий каталог сессии.
3. Ищет строку реестра с самым длинным `repo_path`, совпадающим по границе сегмента пути.
4. **Сам сверяет фактические значения** и при совпадении пропускает команду молча — штатный деплой не требует действий.
5. Расхождение либо невозможность сверить (строки нет, колонка пуста, инструмент не разобран, каталог не определён) → `deny` с обоими значениями. Снимает блокировку пользователь: `CLAUDE_ALLOW_DEPLOY=1`.

Источники сверки, все локальные и быстрые; CLI — только резерв, общий бюджет проб ограничен: Firebase — `.firebaserc`, затем `google-services.json` модуля приложения, затем `firebase use`; Cloudflare — `account_id` из `wrangler.jsonc/json/toml`, затем `wrangler whoami`; GCP — `gcloud config get-value project`; GitHub — `git remote get-url origin` (ssh и https сравниваются по `org/repo`); Play — package из самой команды.

Значения из конфигов сравниваются **строго**: `myapp` не совпадает с `myapp-staging`, иначе деплой в соседний проект того же семейства проходил бы молча. Сырой вывод CLI сверяется по границе токена.

## Что считается опасной командой

`deploy` · `publish` · `release` · `secret set|put` · `functions deploy` · `hosting:channel:deploy` · `run deploy` · `pages deploy` · `apps release` · `gh release create` · `gh repo delete` · `adb uninstall`.

Чтение (`list`, `describe`, `whoami`, `status`, `logs`) не блокируется.
