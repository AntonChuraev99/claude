...
- `hooks/account-align.js` — баг, вскрытый коммитом этой задачи: `commandPositionRe` считал любую `(` позицией команды, и `git commit -m "chore(gcloud): …"` получил `chore(gcloud --account=<почта>): …` — почта в публичном репозитории (коммит amend'нут до push). Первый фикс (скобка — позиция команды только не после буквы) закрыл экземпляр; ревью 2.3b показало класс — `(` после пробела, `;`, `&&` в теле сообщения. Слой класса — «содержимое кавычек не команда»: `maskQuoted` заменяет текст в `"…"`/`'…'` пробелами той же длины (escape `\"` и `` `" ``), позиции ищутся по маске, флаги вписываются в исходную строку. Цена — `"$(gcloud …)"` внутри кавычек не выравнивается (как и `bash -c "…"` до этого; guard при расхождении блокирует). Красные репро: lookbehind — 2 из 40 падают на старом регексе; маска — 3 из 47 падают на версии с одним lookbehind.
