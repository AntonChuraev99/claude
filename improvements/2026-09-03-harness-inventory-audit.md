...
Выполнен с опозданием, 2026-09-23, окно 09-03…09-23 (1447 транскриптов; сырой обход обоих
профилей даёт 2894 — `~/.claude-work/projects` junction на `~/.claude/projects`, каждый файл
считался дважды; следующему замеру обходить один каталог). Промежуточный срез: не сняты always-on
токены (`claude plugin details`, цель ≈6.0k), число MCP-серверов на старте (цель 11), вызовы
`Skill(playwright-cli)` и `SOURCES` дайджестов scout'а — полный replay их ещё должен.

| target | факт |
|---|---|
| вызовы Playwright в веб-задачах | есть: `mcp__plugin_playwright_playwright__*` — 1408 |
| доля `claude-in-chrome` среди браузерных (baseline 100%) | ≈49% (1342 из 2750; источник: разовый скрипт замера, сессия 2026-09-23) — **достигнуто** |
| сервер только с `authenticate` | `cloudflare` снова включён в `enabledPlugins` и живой (`cloudflare-builds` — 52 содержательных вызова), `sentry` — 91 вызов (источник: тот же замер); target «ни одного» выполнен иначе, чем планировался: серверы вернулись, потому что понадобились |
| задачи, упёршиеся в удалённое | не замечено |

Отложенные группы разобраны тем же числом — итог и решения в
