...
**Как ждать.** Ожидание идёт по условию, а не по таймеру: `Monitor` с until-условием на ту же команду проверки состояния — он просыпается по факту, а не по угаданной паузе, и это способ по умолчанию. Цикл опроса — запасной вариант, и он обязан быть ОГРАНИЧЕННЫМ: у Bash-тула потолок 600 000 мс, а CI бывает дольше, поэтому бесконечный `until` не «дождётся», а будет убит по таймауту:

```bash
# GitHub: состояние берётся полем, а не грепом по человекочитаемому выводу
for i in $(seq 1 20); do
  [ "$(gh pr view <num> --json state -q '.state')" = "MERGED" ] && break; sleep 15
done

# GitLab: то же самое через JSON (`glab mr view -F json`), формат строки "State: merged" не контракт
for i in $(seq 1 20); do
  [ "$(glab mr view <iid> -F json | jq -r .state)" = "merged" ] && break; sleep 15
done
```

Не дождались за отведённые итерации — сказать об этом и вернуть контроль, как требует абзац выше, а не крутить цикл дальше.
