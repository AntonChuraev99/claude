...
2. **Дифф непуст.** Наличие исполняемого кода определяет не «запускать или нет», а **сколько агентов**:
   ...
   - **Код есть** → две оси, два параллельных агента (как было). Считается кодом: исходники (`.kt .kts .java .swift .ts .tsx .js .jsx .py .rb .go .rs .php .sql .sh .ps1`), build-скрипты (`*.gradle`, `*.gradle.kts`, `Podfile`, `package.json` в части скриптов и зависимостей) и **конфигурация, исполняемая внешней системой** (`.github/workflows/**`, `.gitlab-ci.yml`, `wrangler.*`, `firebase.json`, `*.tf`, правила безопасности БД).
   - **Кода нет** — правила самого харнесса (`~/.claude/**`), `docs/**`, любые `.md`, `review-rules/*.yaml`, локализация, статические ассеты → **один агент, обе оси в одном брифе, раздельные секции в ответе**. Standards на таком диффе = сверка с документированными правилами (`CLAUDE.md` и его инварианты, конвенции репозитория) плюс целостность ссылок на скиллы, агентов и файлы; **smell baseline Фаулера не применять** — он про код. Spec — без изменений, это единственный механизм, ловящий «сделано не то, о чём просили».
...
**База ревью — вся ветка, а не дифф задачи.** Скоуп `origin/<транк>...HEAD` (транк — из `~/.claude/config/protected-branches.local.json`): на pre-merge прогоне ревьюится то, что реально уедет в MR, включая задачи этой ветки, закрытые прошлыми `lite`-прогонами. Ветка не merge-bound (согласованная работа в транке, репозиторий профиля `~/.claude`) → скоуп прежний, `<BASE_SHA>...HEAD`.

**Как:** fail fast (`git rev-parse <база>`, непустой `git diff --stat <база>...HEAD`) → на диффе с кодом два вызова `Agent(subagent_type="general-purpose")` **одним сообщением**, параллельно; на диффе без кода — один вызов с обеими осями. Отчёты печатать **раздельно по осям**, не сливая и не переранжируя между ними, в обоих случаях.
...
Статус: `✅ clean (2 axes)` / `✅ clean (2 axes, single reviewer — no code in diff)` / `⚠️ N standards + M spec findings` / `❌ spec: требование не реализовано` (блокер — задача не делает того, о чём просили). Standards-находки уровня суждения gate не блокируют; их место — в отчёт.
...
Статус в отчёт: `📦 Ready to merge: <branch> → <trunk>` + строки (2)(3)(4). Репозиторий самого профиля (`~/.claude`, `~/.claude-work`) и согласованная работа в транке → `✅ N/A`, Step 6 пропускается.
...
Табличка `TASK GATE` со всеми пунктами 2.1–2.10 (включая 2.2b и 2.3b) + 3 + 4 + 5.1 + статусами; первой строкой — `Mode: full | lite`, отложенные пункты — строкой `⏸ deferred to pre-merge (full) gate`, завершить `VERDICT: <READY|READY WITH WARNINGS|NOT READY>`. После — обязательно секция Recommendations (5.2.1).
...
**READY / READY WITH WARNINGS →** обновить `task-gate-state.json` в scratchpad: `lastGateSha = HEAD`, once-флаги выполненных `[once]`-проверок (см. Re-entrancy). Затем — блок `📦 Ready to merge` из 5.1a и одна строка запроса подтверждения: `Подтверди результат — дальше сам: push → MR → merge (squash, по зелёному CI) → уборка ветки.` Гарды 5.1a не пройдены → вместо неё причина, почему merge недоступен.
