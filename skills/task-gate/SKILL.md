...
5. **Уборка после подтверждённого merge** — remote-ветка (снята флагом `--remove-source-branch` / `--delete-branch`, иначе `git push origin --delete <branch>`); `git worktree remove .claude/worktrees/<slug>`; `git branch -d <branch>`; основной checkout на транк и `git pull`.
