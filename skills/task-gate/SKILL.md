...
5 шагов **последовательно**, статус (`✅`/`⚠️`/`❌`) после каждой проверки, финальный отчёт в конце: Step 1 — Task Snapshot; Step 2 — Definition of Done Checks (11 пунктов gate); Step 3 — Sub-agent Sanity Check; Step 4 — Recommendations Pull; Step 5 — Commit & Final Report. Дальше скилл **останавливается и ждёт подтверждения пользователя**; получено → Step 6 — MR & Merge (push, MR/PR, merge, уборка).
...
Дифф содержит UI-файлы (Compose-composable, `.tsx`-компоненты, стили) → в отчёте должны быть **названные снимки, которые агент открывал**, и что на них видно. Инвариант CLAUDE.md → «Вёрстка — проверяется картинкой, а не пользователем», процедура — скилл `screenshot-driven-ui`.

- Снимки названы → `✅`.
- Инфраструктуры скриншотов в проекте нет → `⚠️ no screenshot infra` + проверить, что вопрос «заводить сейчас или отложить» вынесен пользователю, а отложенное лежит строкой в `docs/todos/`.
- Ни того, ни другого → `❌` : вёрстка не проверена, а не «проверена ручным сценарием».
...
Табличка `TASK GATE` со всеми пунктами 2.1–2.10 (включая 2.2b и 2.3b) + 3 + 4 + 5.1 + статусами, завершить `VERDICT: <READY|READY WITH WARNINGS|NOT READY>`. После — обязательно секция Recommendations (5.2.1).
