...
**Гейт двухрежимный** (2026-08-19): `full` гоняет всё, `lite` — только проверки, которые ловят баги. Режим выбирается в Step 1, печатается в snapshot и в финальном отчёте. Дальше скилл **останавливается и ждёт подтверждения пользователя**; получено → Step 6 — MR & Merge (push, MR/PR, merge, уборка).
