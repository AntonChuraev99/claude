...
Полный пример payload на 10 параметров (`Submit Purchase Error Support`) и грабли поиска имён параметров в коде (регистр значим, дубли, хардкод) — [`references/reference.md`](references/reference.md).
...
Каждая строка контента — со своим эмодзи в начале (Unicode 🍎🤖🌐 для платформ — работают везде). Полные lookup-таблицы (header по категории event'а + эмодзи по полю/типу данных) — [`references/reference.md`](references/reference.md).
