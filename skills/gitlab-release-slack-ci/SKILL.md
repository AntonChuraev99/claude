...
12 типовых граблей (annotated tag ≠ Release, `#`-заголовки режутся в `-m`, JSON только через `jq`, `needs:` между jobs, Protected variable/tag, CRLF-inflate → Slack `invalid_blocks`, markdown→mrkdwn transform, …) — все закодированы в шаблоне; полный разбор с прецедентами в [`references/pitfalls.md`](references/pitfalls.md).
