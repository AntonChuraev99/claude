...
**Токен умер** (`invalid_grant`, `Token has been expired or revoked`, `Failed to authenticate`) — единственный штатный повод позвать пользователя по поводу аккаунтов. Логиниться самому нельзя; в блок «👉 Сделай руками» идёт точная команда с явным аккаунтом: `gcloud auth login --account=<почта>` либо `firebase login --reauth`.
