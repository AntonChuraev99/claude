...
Что делает хук: находит строку реестра по текущему `cwd`, печатает ожидаемые значения вместе с командой для проверки каждого и **блокирует** вызов (`deny`). Порядок дальше: выполнить читающую команду сверки (она не блокируется), показать пользователю фактический аккаунт рядом с ожидаемым и попросить снять блокировку на сессию — `CLAUDE_ALLOW_DEPLOY=1`. Выставлять эту переменную самому агенту нельзя: это обход проверки, ради которой хук и существует. До 2026-08-04 хук возвращал `ask`, а CLI при `bypassPermissions` его молча проглатывал — то есть защита не работала вовсе. Записи нет — предупреждает, что репозиторий не заведён. Реестра нет вовсе — предупреждает и отправляет создать его из шаблона.
