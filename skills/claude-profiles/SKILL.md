...
Что делает хук: находит строку реестра по текущему `cwd`, печатает ожидаемые значения вместе с командой для проверки каждого и переводит вызов в режим подтверждения. Записи нет — предупреждает, что репозиторий не заведён. Реестра нет вовсе — предупреждает и отправляет создать его из шаблона.
