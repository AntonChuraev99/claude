...
> **ФИНАЛЬНОЕ РЕШЕНИЕ — у агента, которому нужен MCP, не должно быть НИ `tools:`, НИ `disallowedTools:`, НИ inline `mcpServers:`.** Просто ни одной строки про инструменты. Агент наследует серверы активного профиля.
>
> Хронология ниже сохранена намеренно — она показывает, какие гипотезы уже отброшены замером. Два промежуточных варианта (inline `mcpServers:`, затем `disallowedTools:`) в ней разобраны подробно и **оба сняты**.
>
> **Ограничение и MCP взаимоисключающи** — это главный вывод, проверенный форсированными вызовами:
>
> | Механизм | Ограничивает | MCP |
> |---|---|---|
> | `tools:` (allowlist) | **да** | **ноль** |
> | `disallowedTools:` (denylist) | **нет** | да |
> | ничего | нет | да |
>
> `disallowedTools` не соблюдается вообще: `disallowedTools: Grep` → форсированный `Grep` вернул 14 совпадений; `disallowedTools: Bash` → `Bash` вернул `probe-ok`. Записи `Agent`/`NotebookEdit` выглядели рабочими, но при **удалении их из denylist блокировка сохранилась** — вложенные спавны запрещает платформа («Agent is disabled for this session, in subagents as well as here»), а не поле. То есть `disallowedTools` == пустая строка, только с видом гарантии. Удалён у всех 12.
>
> **Что реально держит границы — тело агента.** `jira-expert` дважды отказался вызвать `Bash` («не мой канал работы»), `best-practices-scout` отказался от диагностики вне своего контракта — оба с `tool_uses: 0`, при полностью доступных инструментах. Инструкция в теле работает там, где frontmatter не работает.
