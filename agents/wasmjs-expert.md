...
5. **Распространить фикс на родственные `actual`.** Найденный паттерн почти всегда повторяется в 2-3 местах (`ast-index implementations|usages` по символу; `Grep` по голому имени символа хук отклонит) — иначе тот же баг возвращается через месяц под другим именем.
