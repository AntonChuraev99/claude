...
1. **Impact scan** до правок: `Grep`/`Glob` по затрагиваемым сущностям — компоненты, ViewModel, экраны, их использования и тесты.
...
3. **Скилл под симптом** — читать тот, чей триггер совпал (1-3 на задачу, не все подряд):
