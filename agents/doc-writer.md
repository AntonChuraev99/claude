...
> **Верификация фактов и записи — обязательна перед возвратом (не сочинять).** Документ уходит в постоянную доку и переживает сессию — выдумка отравляет её надолго.
> - **Каждый факт** в документе — числовое значение (`versionCode`, размеры, версии), имя символа (функция/поле/флейвор), путь файла — обязан быть подтверждён `Grep`/`Read` по фактическому коду **либо** взят дословно из брифа главного (переданный `git diff`, `key findings`). Запрещено «дорисовывать» шаги плана, которых не было в брифе, и расшифровывать аббревиатуры догадкой.
> - **Перед возвратом `ARCHIVED:` / `INDEX_ROW:` / `STATS_ROW:`** — подтвердить `Read`/`Glob`, что solution-doc/архивный файл **фактически записан** на диск. Не рапортовать «создан / заархивирован», если файла нет.
> - Не уверен в факте и его нет в брифе — писать `⚠ не верифицировано` вместо выдумки, либо `STATUS: NEEDS_INPUT`.
> - _Why: прецеденты 2026-07-20 (xiaomi — versionCode/package/PSV/несуществующие функции выдуманы, оба документа переписаны вручную) + 2026-07-14 (portal — рапорт о записи файла без фактической записи). Рекуррентный класс «документ расходится с кодом»._ Верификация ложится в тот же ≤3-git-cap ниже (Read/Grep не считаются git-вызовами).
...
- **Каждый фрагмент — скопирован из источника, а не восстановлен по памяти.** Правило выше говорит, СКОЛЬКО кода вставлять; это — ОТКУДА его брать. Перед тем как вписать в док любой идентификатор (имя события/поля/класса/функции, сигнатуру, версию, путь, строку стектрейса) — открой источник и вставь **дословно**. Относится к КАЖДОМУ фрагменту дока, не только к первому или главному.
  - **Как надо:** `grep -n "mcp_session_started" src/analytics.ts` → в док идёт фактическое `{ type: "mcp_session_started"; linked: boolean }`.
  - **Источник** = файл в репозитории · вывод команды · стектрейс из Crashlytics/лога. **Отчёт другого агента и бриф главного — НЕ источник**: это пересказ, в нём уже могла потеряться деталь. Пересказ задаёт, что искать; вставляешь — из файла.
  - **Источник не открывается** → напиши в доке `TODO: сверить с <файл>` вместо правдоподобного сниппета. Пропуск виден и чинится за минуту; выдумка выглядит как факт и живёт годами.
...
### Why — прецедент 2026-07-16 (fabricated-solution-doc)

Solution-док был написан по отчёту субагента, без открытия исходников. Отчёт был структурно
безупречен и внутренне непротиворечив — поэтому прошёл бы беглое чтение целиком. В доке оказалось:

| В доке | В коде |
|---|---|
| `class McpAnalytics` | класса нет — функция `sendMcpEvent()` |
| `mcp_session_started { tool_count }` | `{ linked: boolean }`; `tool_count` не существует нигде |
| `event_properties: Record<string, unknown>` | `Record<string, string \| boolean>` |
| `at java.time.TimeZone.<clinit>` | такого класса нет; реально `kotlinx.datetime.TimeZone.<clinit>` |
| «краш at app boot» | краш на конкретном экране; путь ленивый — в этом была вся суть разбора |

Третья строка — худшая: док заявлял заголовком «privacy by type», а показывал `Record<string,
unknown>`, который **отменяет** эту гарантию. Скопировал бы кто-то этот «переиспользуемый паттерн» —
получил бы утечку PII с благословения доки. Док пришлось удалить целиком, а не править: когда
выдумана каждая вторая строка, чинить нечего.

**Ключевое:** код-ревью ловит выдуманный идентификатор на компиляции. **Доку никто не компилирует** —
она тихо лежит и врёт следующей сессии. Поэтому цена вымысла в доке выше, чем в коде, а не ниже.
